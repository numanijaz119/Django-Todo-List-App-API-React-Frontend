...
class HomePage extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (this.state.editing === true) {
      url = `http://127.0.0.1:8000/api/tasks/update-task/${this.state.activeItem.id}/`;
      this.setState({
        editing: false
      });
    }
    ...
    const taskData = {
      ...this.state.activeItem,
      user: this.context.user // Include the user in the task data
    };
  ...
  deleteItem(task) {
    ...
    fetch(`http://127.0.0.1:8000/api/tasks/delete-task/${task.id}/`, {
      method: 'DELETE',
      headers: {
        'Content-type': 'application/json',
        'X-CSRFToken': csrftoken,
  ...
  strikeUnstrike(task) {
    ...
    var csrftoken = this.getCookie('csrftoken');
    var url = `http://127.0.0.1:8000/api/tasks/update-task/${task.id}/`;
